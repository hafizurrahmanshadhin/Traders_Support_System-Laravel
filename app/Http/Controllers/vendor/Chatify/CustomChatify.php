...
class CustomChatify extends Controller
{
    ...
    public function send(Request $request)
    {
        ...
        if (Auth::check() && Auth::user()->role == 'trade') {
            ...
            if (!$membership) {
                $error->status = 1;
                $error->message = "Your subscription is not active. Please, purchase a subscription to continue.";
            }

            $myConnection = MyConnect::where(['user_id' => auth()->user()->id, 'connect_id' => $request['id']])->first();
            if (!$myConnection) {

                $Limite = Subscription::where('id', $membership->subscription_id)->pluck('view_limit')->first();
                if ($membership->profile_message_used >= $Limite) {

                    $error->status = 1;
                    $error->message = "You have exceeded your Message limit. You need to purchase another subscription to view more profiles.";
                } else {
                    // Create New And Increment
                    $myConnection = new MyConnect();
                    $myConnection->user_id = auth()->user()->id;
                    $myConnection->connect_id = $request['id'];
                    $myConnection->save();

                    //! Increment the profile views count
                    $membership->increment('profile_message_used');
                    $user->increment('profile_message');
                }
            }
        }
